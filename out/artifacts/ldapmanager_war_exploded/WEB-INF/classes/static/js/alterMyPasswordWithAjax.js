...
function changePassword() {
	...
	//alert('tipo de usuário: '+ alterMyPasswordModel.typeOfUser);
	var alterMyPasswordModelJson = JSON.stringify(alterMyPasswordModel);
	...
	if (validationPassword(alterMyPasswordModel.newPassword)) {
		$.ajax({
			type: 'POST',
			contentType: 'application/json',
			url: 'http://' + host + '/altermypassword/changepassword',
			dataType: "json",
			data: alterMyPasswordModelJson,
			cache: false,
			async: false,
			...
			error: function(jqXHR, textStatus, errorThrown) {
				...
				alert('erro ao alterar a senha Código de erro: ' + jqXHR.status + '. Mensagem de erro: ' + jqXHR.statusText);
